...
# Endpoint to change  role to a user
@router.post("/users/{username}/roles/{role_id}", response_model=user_sch.UserResponse)
def change_role_to_user(username: str, role_id: int, db: Session = Depends(get_db)):
    user = user_crud.change_role_to_user(db=db, username=username, role_id=role_id)
    return user  # Return the updated user response
